...
/**
 * Clase de vista que genera una ventana de ficha de alumno, donde podremos modificar los valores de un
 * alumno, asignarlo a un grupo, darlo de baja o utilizar los valores para generar un alumno nuevo
 * 
 * @author Rubén Torrejón Espinosa
 *
 */
@SuppressWarnings("serial")
public class VAlumnoFicha extends JFrame implements ActionListener, MouseListener{
	...
	private JButton Btn_Logout, Btn_Inicio, Btn_Config, Btn_Ayuda, Btn_Limpiar, 
		Btn_Registrar, Btn_Volver, Btn_Buscar, Btn_Modificar, Btn_Asignar;
	...
	private JLabel lbl_Form_0, lbl_Form_1, lbl_Form_2, lbl_Form_3, lbl_Form_4, lbl_Form_5, 
		...
		lbl_Form_12, lbl_Form_13, lbl_Form_14;	
	...
	private JTextField TF_Nombre, TF_Apellidos, TF_Dni, TF_Telefono, TF_Email, TF_Direccion, 
		TF_NombreResp, TF_DniResp, TF_TelefonoResp, TF_EmailResp, TF_FechaAlta, TF_FechaBaja;
	 ...
	 */
	public VAlumnoFicha(Empleado miEmpleado) {
	...
	/**
	 * Constructor con parámetro alumno
	 * 
	 * @param miEmpleado
	 * @param miAlumno
	 */
	public VAlumnoFicha(Empleado miEmpleado, Alumno miAlumno) {
		getContentPane().setBackground(new java.awt.Color(27, 38, 79));
		this.miEmpleado = miEmpleado;
		this.miAlumno = miAlumno;
		initialize();
		rellenarComponentesDeAlumno(miAlumno);
	 ...
	 * @param miAlumno
	 */
	private void rellenarComponentesDeAlumno(Alumno miAlumno) {
		TF_Nombre.setText(miAlumno.getNombre());
		TF_Apellidos.setText(miAlumno.getApellidos());
		TF_Dni.setText(miAlumno.getDni());
		TF_Telefono.setText(miAlumno.getTelefono());
		TF_Direccion.setText(miAlumno.getDireccion());
		TF_Email.setText(miAlumno.getEmail());
		TF_NombreResp.setText(miAlumno.getResponsableNombre());
		TF_DniResp.setText(miAlumno.getResponsableDni());
		TF_TelefonoResp.setText(miAlumno.getResponsableTelefono());
		TF_EmailResp.setText(miAlumno.getResponsableEmail());
		if(miAlumno.getFechaNacimiento() != null) {
			CB_DiaNac.setSelectedItem(miAlumno.getDay(miAlumno.getFechaNacimiento()));
			CB_MesNac.setSelectedItem(rellenador.pasaNumeroAMes(miAlumno.getMonth(miAlumno.getFechaNacimiento())));
			CB_YearNac.setSelectedItem(miAlumno.getYear(miAlumno.getFechaNacimiento()));
		} else {
			CB_DiaNac.setSelectedItem("Día");
			CB_MesNac.setSelectedItem("Mes");
			CB_YearNac.setSelectedItem("Año");			
		}
		TF_FechaAlta.setText(miAlumno.getFechaAlta().toString().substring(8,10)
							+miAlumno.getFechaAlta().toString().substring(4,8)
							+miAlumno.getFechaAlta().toString().substring(0,4));
		if(miAlumno.getFechaBaja() != null) {
			TF_FechaBaja.setText(miAlumno.getFechaBaja().toString().substring(8,10)
								+miAlumno.getFechaBaja().toString().substring(4,8)
								+miAlumno.getFechaBaja().toString().substring(0,4));
		}
	}
	...
	private void incluirComponentes() {
		...
		JPanel_Botones.add(Btn_Asignar);		
	...
	private void establecerFuentes() {
		...
		Btn_Asignar.setFont(new Font("Times New Roman", Font.BOLD, 25));
	...
	private void posicionarComponentes() {
		...
		Btn_Limpiar.setBounds(265, 420, 150, 75);
		Btn_Buscar.setBounds(585, 420, 150, 75);
		Btn_Modificar.setBounds(425, 420, 150, 75);
		Btn_Asignar.setBounds(745, 420, 150, 75);
		...
		CheckBaja.setBounds(895, 355, 115, 40);
	...
	private void establecerColores() {
		...
		Btn_Asignar.setForeground(Color.BLACK);
		Btn_Asignar.setBackground(new Color(255, 255, 127));
	}
	...
	private void iniciarComponentes() {
		...
		Btn_Registrar = new JButton("Nuevo");
		Btn_Registrar.setText("<html><p>Registrar</p> <p>Nuevo</p></html>");
		...
		lblAlumnos = new JLabel("Alumnos - Ficha");
	...
	public void actionPerformed(ActionEvent e) {
		...
		switch (txtBtn) {
			case "<html><p>Registrar</p> <p>Nuevo</p></html>":
				...
				if(!comprobador.esUnDni(TF_DniResp.getText()))
					...
					dniResponsable = true;
				if(miAlumnoDao.alumnoExiste(TF_Dni.getText()))
			...
			case "Buscar":
				ArrayList<Alumno> listaAlumnos = miAlumnoDao.buscarAlumnos(
						TF_Nombre.getText(),TF_Apellidos.getText(),TF_Dni.getText(),
						TF_Telefono.getText(),TF_Direccion.getText(),TF_Email.getText(),
						TF_NombreResp.getText(),TF_DniResp.getText(),TF_TelefonoResp.getText(),
						TF_EmailResp.getText());
				if(listaAlumnos.size()==1) {
					Btn_Modificar.setEnabled(true);
					miAlumno = listaAlumnos.get(0);
					rellenarComponentesDeAlumno(miAlumno);
					Btn_Asignar.setEnabled(true);
					...
					JOptionPane.showMessageDialog(this, "Lo sentimos, no se ha encontrado ningún alumno con esas características en la BBDD");
					reiniciaLabels();
				}
				break;
			case "Asignar":
				this.setVisible(false);
				this.dispose();
				alumnoAsignar = new VAlumnoAsignar(miEmpleado, miAlumno);
				break;
			case "Modificar":
				Boolean fechaCorrectaM = false;
				Boolean baja = false;
				...
				if(!comprobador.esUnDni(TF_DniResp.getText()))
					...
					dniResponsable = true;
				if(telefonoAlumno && telefonoResponsable && dniAlumno && dniResponsable) {
					if(fechaCorrectaM) {
						String diaM = CB_DiaNac.getSelectedItem().toString(); 
						String mesM = rellenador.pasaMesANumero(CB_MesNac.getSelectedItem().toString());
						String yearM = CB_YearNac.getSelectedItem().toString();
						String fechaNacM = yearM+"-"+mesM+"-"+diaM;
						miAlumno = miAlumnoDao.modificarAlumno(
							TF_Nombre.getText(),TF_Apellidos.getText(),TF_Dni.getText(),
							TF_Telefono.getText(),TF_Direccion.getText(),TF_Email.getText(),
							Date.valueOf(fechaNacM),TF_NombreResp.getText(),TF_DniResp.getText(),
							TF_TelefonoResp.getText(),TF_EmailResp.getText(),miAlumno,baja);
						JOptionPane.showMessageDialog(this, "Datos del alumno modificados con éxito");
					} else {
						miAlumno = miAlumnoDao.modificarAlumno(
							TF_Nombre.getText(),TF_Apellidos.getText(),TF_Dni.getText(),
							TF_Telefono.getText(),TF_Direccion.getText(),TF_Email.getText(),
							null,TF_NombreResp.getText(),TF_DniResp.getText(),
							TF_TelefonoResp.getText(),TF_EmailResp.getText(),miAlumno,baja);
						JOptionPane.showMessageDialog(this, "Datos del alumno modificados con éxito");
					}
				}
			...
			case "Volver":
				this.setVisible(false);
				this.dispose();
				alumnoBuscarTodos = new VAlumnoBuscarTodos(miEmpleado, miAlumnoDao.buscarAlumnos());
				break;
			case "Limpiar":
				reiniciaLabels();
				Btn_Modificar.setEnabled(false);
